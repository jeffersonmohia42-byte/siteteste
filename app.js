...
export function initApp({ generateLTC, getNoiseBaseURL, fetchAudioBuffer }) {
    ...
    document.getElementById('btnProcess').onclick = async () => {
      ...
      if (modeL.value === 'noisebase') {
        // 1) baixa ruído base da nuvem
        const url = await getNoiseBaseURL();
        const base = await fetchAudioBuffer(url, audioCtx);
        // 2) resample se necessário
        let baseBuf = base;
        ...
        const pulse = Math.floor(sr * 0.04);
        for (let start = 0; start < len; start += beat) {
          for (let i = 0; i < pulse && start + i < len; i++) {
            const env = Math.exp(-i / (sr * 0.02)); // 20ms decay
